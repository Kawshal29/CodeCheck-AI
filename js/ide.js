// const API_KEY = "7a7845b0femsh9fe4be154dc7ce7p114fccjsnc671f0a98c70";
const API_KEY = "21c517c2e5mshbb56ab2dde03ce1p18f34fjsne0f6bb5dddaf";
...
//new feature converter
const API_KEY2 = "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";

// to convert code from one language to a another
document.addEventListener("DOMContentLoaded", function () {
  let selectElementMain = document.getElementById("select-language");
  let selectElement = document.getElementById("selected-lang2");

  selectElementMain.addEventListener("change", function () {
    //   let selectElement = document.getElementById("selected-lang2");
    let $dropdown2 = $("#selected-lang2");
    console.log("Dropdown element:", $dropdown2);

    // Ensure dropdown is initialized before setting selected value
    $dropdown2.dropdown();

    // Set the selected option
    $dropdown2.val(selectElementMain.value).change(); // Ensure to trigger change event
  });

  // Event listener for change event
  selectElement.addEventListener("change", function () {
    // $("#selected-lang2").addClass("loading");
    ...
    let userMessage = `${sourceEditor.getValue()} convert to ${
      languageMappings[selectedValue]
    } dont give description`;

    console.log("selected_lang2", languageMappings[selectedValue]);
    // const messageElement = document.querySelector("#commandTextResponse");
    console.log("input value->", sourceEditor.getValue());
    ...
    fetch(API_URL, requestOptions)
      ...
      .then((data) => {
        ...
        sourceEditor.setValue(response);

        let $dropdown = $("#select-language");
        ...
        // Set the selected option
        $dropdown.val(selectedValue).change(); // Ensure to trigger change event
        // $("#selected-lang2").removeClass("loading");
      })
      .catch(() => {
...
//to convert from text to code
function convert() {
  const API_URL = "https://api.openai.com/v1/chat/completions";
  let userMessage = document.querySelector("#commandText").value;
  // document.querySelector("#convert").addClass("loading");
  $("#convert").addClass("loading");
  const selected_lang2 = document.querySelector("#selected-lang3").value;
  ...
  userMessage += `${languageMappings[selected_lang2]} dont give description`;

  console.log("selected_lang2", languageMappings[selected_lang2]);
  // const messageElement = document.querySelector("#commandTextResponse");
  console.log("input value->", sourceEditor.getValue());
  // Define the properties and message for the API request
  const requestOptions = {
  ...
  fetch(API_URL, requestOptions)
    ...
    .then((data) => {
      ...
      // Set the selected option
      $dropdown.val(selected_lang2).change(); // Ensure to trigger change event
      let $dropdown2 = $("#selected-lang2");
      console.log("Dropdown element:", $dropdown2);
      ...
      // Set the selected option
      $dropdown2.val(selected_lang2).change(); // Ensure to trigger change event
      $("#convert").removeClass("loading");
    })
    .catch(() => {
      console.log("Oops! Something went wrong. Please try again.");
      $("#convert").removeClass("loading");
...
var competitiveProgrammingSource = `#include <iostream>
using namespace std;
  int main() {
      cout<<"Hello World"; \n
}`;
